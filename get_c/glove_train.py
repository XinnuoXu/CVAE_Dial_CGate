...
class GloVeFilter(object):
	...
	def filter(self, tag="test"):
		en_list = [l.strip().decode("utf8", "ignore") for l in open("./examples/" + tag + ".en")]
		vi_list = [l.strip().decode("utf8", "ignore") for l in open("./examples/" + tag + ".vi")]
		...
		for (en, vi, enp, vip, discount) in test_data:
		    print (en + "\t" + vi + "\t" + str(self._paragraph_similarity(enp, vip)))
